...
model {
  ...
  for (i in 1:N_short) {
    if (time_short[i] < step_time) {
      value_short[i] ~ normal(alpha_short + beta_short_before * time_short[i], sigma_short);
    } else {
      value_short[i] ~ normal(alpha_short + beta_short_before * step_time + beta_short_after * (time_short[i] - step_time), sigma_short);
    }
  }
}
